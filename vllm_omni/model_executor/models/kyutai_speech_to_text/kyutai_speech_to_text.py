...
class KyutaiSpeechToTextForConditionalGeneration(
    ...
    def _compute_audio_bias_full(self, input_values: torch.Tensor, device: torch.device) -> torch.Tensor:
        ...
        Output shape ``(1 + T, hidden_size)``: index 0 holds the audio-BOS
        bias (used at the BOS position); indices 1..T hold the per-frame
        biases. The dtype follows the codec parameters (typically fp32 even
        when the LM runs in bf16) so the per-step LM add keeps the higher-
        precision summation; the runner casts back to its bf16 buffer at
        write-time.
        """
    ...
    def kyutai_preprocess(
        ...
        **info: object,
        ...
        On the first call the audio waveform is read from ``mm_features``,
        encoded by the Mimi codec, and the per-position bias buffer
        ``_kyutai_audio_bias_full`` is stashed in the model_intermediate_buffer.
        Subsequent calls just slice into that buffer using ``_kyutai_next_pos``.
        Past the audio horizon the bias is zero (the LM then samples
        unconditioned text); callers should cap ``max_tokens`` at the audio
        horizon to avoid that.
        """
        ...
        end = next_pos + input_embeds.shape[0]
        if end <= bias_full.shape[0]:
            # Whole-tensor add keeps the result in the higher-precision codec
            # dtype until the runner casts back to its bf16 buffer; per-slice
            # assignment would quantize mid-add.
            input_embeds = input_embeds + bias_full[next_pos:end]
        else:
            slice_len = bias_full.shape[0] - next_pos
            if slice_len > 0:
                input_embeds = input_embeds.clone()
                input_embeds[:slice_len] = input_embeds[:slice_len] + bias_full[next_pos:]
